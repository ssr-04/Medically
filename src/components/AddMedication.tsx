...
import { addMedicationAction } from "@/actions/medication.action";
...
export default function AddMedicationDialog({conditionId, mutate}:{conditionId:number, mutate: ()=>void}) {
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
        const result = await addMedicationAction(medications,conditionId);
        if(result){
            toast.success('Condition added suceesfully')
            mutate();
            setMedications([
                {
                  name: "",
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <Card className="shadow-md hover:bg-zinc-800 hover:border hover:border-zinc-400">
        <CardContent className="flex justify-center py-12 gap-2 text-muted-foreground">
            <PlusIcon />
            Add Medication
        </CardContent>
    </Card>
      ...
      <DialogContent className="w-full sm:max-w-[600px] h-[80vh]">
        <ScrollArea className="h-full">
          ...
          <DialogHeader className="sticky top-1 bg-background z-10 border-b pb-2">
            ...
            <DialogDescription>
              Enter the details for the medication.
            </DialogDescription>
          </DialogHeader>
